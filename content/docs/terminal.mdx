...
interface TerminalProps {
  ...
  resultStatusMessage?: React.ReactNode; 
  autoStart?: boolean;
}
...
export function Terminal({
  ...
  resultStatusMessage = 'Success! Command executed.',
  autoStart = true,
}: TerminalProps) {
    ...
    const processCommand = async () => {
      ...
      const typeChar = () => {
        if (charIndex < currentCommand.cmd.length) {
          ...
          }, 300); // Delay after typing before output
        }
      };
              ...
              >
               ...
               {showResultStatus && (
                <motion.div
                  ...
                  className="pl-5 text-green-500/80 text-xs mt-2 mb-4"
                >
                  {resultStatusMessage}
                </motion.div>
              )}
